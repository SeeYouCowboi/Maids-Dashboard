...
import { Activity, BarChart3, Cpu, HeartPulse, Radio, ScrollText, Users } from 'lucide-react'
...
/* ── Log types (mirrors GET /v1/logs response) ─────────────────────────── */
...
export default function ObservatoryPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Observatory" subtitle="System health, activity overview, weekly trends">
      ...
      <GlassCard title="Recent Log Activity" color="purple">
        {recentLogs.length === 0 ? (
          <EmptyState icon={<ScrollText className="w-8 h-8" />} message="No recent log entries" />
        ) : (
      ...
      <GlassCard color="purple">
        <div className="flex items-start gap-3 text-sm">
          ...
          <p className="text-gray-500 font-medium leading-relaxed">
            <span className="font-bold text-purple-600">v2 Metrics Pipeline</span>
            {' — '}
            Detailed metrics pipeline coming in v2. Current data is aggregated from live gateway
            endpoints and may not reflect historical trends beyond the active session window.
          </p>
        </div>
      </GlassCard>
    </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        <div className={`p-2 rounded-xl ${style.bg} ${style.text} ring-1 ${style.ring}`}>
          {icon}
        </div>
...
function formatLogTs(unix: number): string {
...
function TimelineEntry({ entry }: { entry: LogEntryRow }) {
  const levelClass = LOG_LEVEL_COLORS[entry.level] ?? 'text-gray-500 bg-gray-50'

  const content = (
    <li className="flex items-start gap-3 py-2.5 px-1 group">
      <span
        className={`shrink-0 mt-0.5 text-[10px] font-bold uppercase rounded px-1.5 py-0.5 ${levelClass}`}
      >
        {entry.level}
      </span>
      <div className="min-w-0 flex-1">
        <p className="text-sm text-gray-700 truncate">{entry.message}</p>
        <p className="text-xs text-gray-400 mt-0.5">{formatLogTs(entry.timestamp)}</p>
      </div>
    </li>
  )
  ...
  if (entry.request_id) {
    return (
      <Link
        to={`/war-room?tab=event-stream&subtab=failed-requests&request_id=${encodeURIComponent(entry.request_id)}`}
        className="block hover:bg-purple-50/60 rounded-lg transition-colors"
      >
        {content}
      </Link>
    )
